...
function buildGoogleSheetsRow(card) {
  return [
    card.createdAt,
    card.mode,
    card.id,
    ...
    card.residence,
    card.employmentType,
    card.seniorityMonths,
    card.netIncome,
    card.otherIncome,
    card.currentPayments,
    card.creditHistory,
    card.housing,
    card.savings,
    card.loanAmount,
    ...
    card.purpose,
    card.notes || "",
    Number(card.currentDti || 0).toFixed(1),
    Number(card.totalDti || 0).toFixed(1),
    Number(card.monthlyPayment || 0).toFixed(2),
    card.total,
    card.approved ? "Так" : "Ні",
    card.risk,
    card.offer,
    `${card.amountAdvice}; ${card.verification}`
  ];
}

async function saveCardToGoogleSheets(card) {
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(message || `Google Sheets sync failed: ${response.status}`);
  }

  const result = await response.json();
  return { saved: result.synced > 0, result };
}

async function saveCardToCloud(card) {
  const supabase = await saveCardToSupabase(card);
  const googleSheets = supabase.saved ? await saveCardToGoogleSheets(card) : { saved: false };

  return {
    saved: supabase.saved || googleSheets.saved,
    supabase: supabase.saved,
    googleSheets: googleSheets.saved
  };
}
    ...
    <>
      <header className="site-header">
        <a className="brand" href="#top" aria-label="Veris">
          <span className="brand-word">Veris</span>
